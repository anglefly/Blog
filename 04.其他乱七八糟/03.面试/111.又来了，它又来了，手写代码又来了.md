...
1. 取得当前实例对象原型链的原型链（`proto = proto.____proto____`，沿着原型链一直向上查找）
1. 如果 当前实例的原型链`____proto__`上找到了当前类的原型`prototype`，则返回 `true`
1. 如果 一直找到`Object.prototype.____proto____ == null`，`Object`的基类(`null`)上面都没找到，则返回 `false`
...
- 【第二次触发：假设时间间隔是`500ms`，第一次执行完之后，`20ms`之后，立即触发第二次，则`remaining = 500 - ( 新的当前时间 - 上一次触发时间 ) = 500 - 20 = 480`】
```js
/**
 * 实现函数的节流 （目的是频繁触发中缩减频率）
 * @param {*} func 需要执行的函数
...
- 步骤1：创建一个`Func`的实例对象（`实例.__proto__ = 类.prototype`）
...
```
## 14.使用`reduce`手动实现数组扁平化`flat`方法
...
```
## 16.基于`Generator`函数实现`async/await`原理
...
## 30.手动实现`Promise.prototype.finally`
